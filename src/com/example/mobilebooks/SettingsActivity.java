...
public class SettingsActivity extends PreferenceActivity implements
	...
	void updateListPreference(ListPreference preference) {
		...
		if (preference.getKey().equals("camera_switch_visibility")) {
			...
			getPreferenceScreen().findPreference("camera_switch_button_x")
					.setEnabled(enabled);
			getPreferenceScreen().findPreference("camera_switch_button_y")
					.setEnabled(enabled);
		}
	}
	...
	public static void setSettingsForPicker(Context context, ScanditSDK picker) {
		...
		picker.setEan13AndUpc12Enabled(prefs.getBoolean(
				"ean13_and_upc12_enabled", true));
		...
		picker.setMsiPlesseyEnabled(prefs.getBoolean("msi_plessey_enabled",
				false));
		picker.setGS1DataBarEnabled(prefs.getBoolean("databar_enabled", false));
		picker.setGS1DataBarExpandedEnabled(prefs.getBoolean(
				"databar_expanded_enabled", false));

		int msiPlesseyChecksum = Integer.valueOf(prefs.getString(
				"msi_plessey_checksum", "1"));
		...
		picker.setDataMatrixEnabled(prefs.getBoolean("data_matrix_enabled",
				true));
		if (prefs.getBoolean("data_matrix_enabled", true)) {
			picker.setMicroDataMatrixEnabled(prefs.getBoolean(
					"micro_data_matrix_enabled", false));
			picker.setInverseRecognitionEnabled(prefs.getBoolean(
		...
		picker.restrictActiveScanningArea(prefs.getBoolean(
				"restrict_scanning_area", false));
		picker.setScanningHotSpotHeight(prefs.getInt("hot_spot_height", 25) / 100.0f);
		picker.getOverlayView().drawViewfinder(
				prefs.getBoolean("draw_viewfinder", true));
		...
		picker.getOverlayView().setBeepEnabled(
				prefs.getBoolean("beep_enabled", true));
		picker.getOverlayView().setVibrateEnabled(
				prefs.getBoolean("vibrate_enabled", true));

		picker.getOverlayView().showSearchBar(
				prefs.getBoolean("search_bar", false));
		picker.getOverlayView().setSearchBarPlaceholderText(
				prefs.getString("search_bar_placeholder",
						"Scan barcode or enter it here"));

		picker.getOverlayView().setTorchEnabled(
				prefs.getBoolean("torch_enabled", true));
		...
		int cameraSwitch = Integer.valueOf(prefs.getString(
				"camera_switch_visibility", "0"));
		...
		picker.getOverlayView().setCameraSwitchVisibility(
